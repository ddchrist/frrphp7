...
elseif(isset($_GET["StatusFilterSubmit"])) {
...
function ShowListOfPCRs()
{
   ...
   foreach ($options as $opt) {
      ...
      $link= '<input type="checkbox" name="check_status[]" value="'.$opt.'" '.$checked.'/>'.$opt;
   ...
   echo generateSelect2('ClosureReason', $options, $ClosureReason);
   ...
   SafetyAnalysed: Problem is Safety Analysed and no longer considered an issue.">ClosureReason</span>';
      
   $query=array('$or'=>$query);

   // ClosureReason handling
   if($ClosureReason=="NO_REASON" && $StatClo) {
//      $query=array('$and'=>array($query,array('$or'=>array(array("ClosureReason"=>"")),
...
function ViewTrackOfPCR($Id)
{
   ...
   $selected=$Status;
   echo "<td>".generateSelect2('Status', $options, $selected)."</td>";
...
// if $value starts with '*' it will be the selected in the drop down
   $html = '<select name="'.$name.'" id="PCRStatus" onchange="SelectPCRStatus()">';
